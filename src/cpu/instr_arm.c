...
{ \
	...
	{ \
		...
		uint8_t rm = cpu->instr_opcode & 0xF; \
		if (post_pre) \
			snprintf(data, size, #opname " r%d, [r%d, %sr%d," #rotname " #0x%x]%s", rd, rn, dec_inc ? "" : "-", rm, shift, writeback_mm ? "!" : ""); \
		else \
			snprintf(data, size, #opname " r%d, [r%d], %sr%d, " #rotname " #0x%x", rd, rn, dec_inc ? "" : "-", rm, shift); \
	} \
	else \
	{ \
