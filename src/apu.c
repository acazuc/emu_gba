...
static uint8_t channel3(apu_t *apu)
{
	...
	if (apu->wave3_gain & 0x4)
		return (sample >> 2) + (sample >> 4);
	return sample >> gain_shifts[apu->wave3_gain];
}
...
void apu_start_channel2(apu_t *apu)
{
	...
	apu->wave1_env.step = 0;
	apu->wave1_env.time = (cnt_l >> 8) & 0x7;
	apu->wave1_env.val = (cnt_l >> 12) & 0xF;
	apu->wave1_env.dir = (cnt_l >> 11) & 0x1;
	apu->wave1_nb = (1 << 2) * (2048 - (cnt_h & 0x7FF));
	apu->wave1_len = 64 - (cnt_l & 0x3F);
	apu->wave1_haslen = (cnt_h >> 14) & 1;
	apu->wave1_duty = (cnt_l >> 6) & 0x3;
}
